...
public class AVLPractice<T extends Comparable<T>>{
    ...
    private T retrieveFromTree(T id, AVLNode<T> focusNode){
        while(!id.equals(focusNode.getData())){
            if(isLeftSide(id, focusNode.getData())) focusNode = focusNode.getLeftSide();
            else if(isRightSide(id, focusNode.getData())) focusNode = focusNode.getRightSide();
            if(isNull(focusNode)) return null;
        }
        return focusNode.getData();
    }
